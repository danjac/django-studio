...
4. Runs `.agents/skills/djs-db-restore/scripts/db-restore.sh <filename>` — suspends CronJobs, scales down app and worker, takes a safety backup, restores in-cluster, scales back up, resumes CronJobs (even if interrupted).
...
- `/djs-enable-db-backups` — set up automated backups if not yet configured
- `/djs-db-backup` — trigger a manual backup immediately
