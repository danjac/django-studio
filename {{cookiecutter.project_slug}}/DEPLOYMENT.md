...
Fresh install of {{cookiecutter.project_name}} on a Hetzner Cloud K3s cluster with Cloudflare CDN/SSL.
...
```bash
just get-kubeconfig
kubectl --kubeconfig ~/.kube/{{cookiecutter.project_slug}}.yaml get nodes  # sanity check
```
...
```bash
base64 -w0 ~/.kube/{{cookiecutter.project_slug}}.yaml
# macOS: base64 -i ~/.kube/{{cookiecutter.project_slug}}.yaml
```
