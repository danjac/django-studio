...
| `release.sh` fails with `unrecognized arguments: --no-input` during `migrate` | django-tenants overrides the `migrate` command and only accepts the short form `--noinput`; Django's own `--no-input` alias is not recognised | Use `--noinput` instead of `--no-input` in `release.sh` and any other scripts that call `migrate` |
