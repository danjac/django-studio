**/djs-tailscale [enable|status|disable]**
...
- **Retrofit** — the cluster is already running. Existing nodes cannot pick this
  ...
  `.agents/skills/djs-tailscale/scripts/join-nodes.sh` over SSH instead. On the
...
```bash
/djs-tailscale                 # is it on, and is the firewall still open?
/djs-tailscale enable          # set it up, or retrofit a running cluster
/djs-tailscale disable         # unwind it safely
```
