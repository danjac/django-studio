...
`docs/this-project.md`, which `/dj-bootstrap` writes: the `User` model, domain apps
and models, languages, and the rest of the project docs. This skill only
...
Work from the project root for the rest of this skill. It must be a django-studio
project with at least one commit:

```bash
test -f .copier-answers.yml && test -d .agents/skills && git rev-parse --verify HEAD
git status --porcelain
just start --wait
```

Stop if it isn't a django-studio project or has no commits. If the working tree
has uncommitted changes, ask whether to continue: this skill ends with a commit
...
must also be handled in `<package_name>/users/gdpr.py`; see `docs/gdpr.md`. Then:
