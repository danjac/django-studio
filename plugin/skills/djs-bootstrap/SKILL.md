...
`copier update` and `/djs-sync` work on it.
...
`AGENTS.md` tells agents to read it, and `/djs-kickoff` and `/djs-doc` build on it.

Follow `<target>/.agents/skills/djs-kickoff/references/overview.md` for the
...
reporting it with `/djs-feedback` from inside the project. Don't edit generated files
...
Ask whether to kick off the project now: the project's `/djs-kickoff` skill turns
`docs/this-project.md` into a user model, domain apps and models, and languages.
If yes, follow `<target>/.agents/skills/djs-kickoff/SKILL.md` from the target
...
1. Quit Claude Code, then `cd <target>` (when it is a subdirectory) and start
   Claude Code again. The project's own `/djs-*` skills and docs load only in a
   session started in the project.
2. `/djs-kickoff`, if it didn't run above.
...
6. `/djs-deploy` when you are ready to go live.
