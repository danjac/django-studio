...
> `/djs-scale` when you need more capacity.
...
> You can skip this now and run `/djs-deploy-observe` later to add it. (y/n)
...
If **n**: tell the user:
> Monitor VM skipped. Run `/djs-deploy-observe` at any time to provision
...
If **y**: tell the user to run `/djs-tailscale enable` **after** this wizard
...
until `kubectl` has been verified over the tailnet — `/djs-tailscale` sequences
that correctly.
...
`/djs-tailscale enable` can retrofit a running cluster later.
...
3. Wait for the domain to show as **Active**, then re-run `/djs-deploy`
...
```yaml
secrets:
  # auto-generated — rotate with: /djs-rotate-secrets
...
is configured later by `/djs-deploy-observe` — do not prompt for it here.
...
> **Next steps (optional):**
> - Run `/djs-scale [n]` to view or change the webapp replica count
> - Run `/djs-enable-db-backups` to set up automated daily PostgreSQL backups
> - Run `/djs-rotate-secrets` to rotate auto-generated secrets when ready

If `<create_monitor>` is `true`, also include:

> - Run `/djs-deploy-observe` to deploy Grafana + Prometheus + Loki

If `<create_monitor>` is `false`, also include:

> - Run `/djs-deploy-observe` to provision the monitor VM and deploy Grafana + Prometheus + Loki
