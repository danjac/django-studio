...
1. Check whether `<package_name>/<app_name>/` exists as a directory.
   ...
   Then read `.agents/skills/djs-create-app/SKILL.md` and execute those steps
   for `<app_name>` before continuing.

2. Read `<package_name>/<app_name>/models.py`. If `<model_name>` is not defined
   there, tell the user:
   > `<model_name>` does not exist yet. Starting `djs-create-model` now.
   Then read `.agents/skills/djs-create-model/SKILL.md` and begin that flow.
   **`djs-create-model` is interactive — it will ask the user for field definitions
   and wait for their response. Do not skip Step 1 of `djs-create-model`. Do not
   invent fields.** Only resume CRUD generation after `djs-create-model` has fully
...
**Fixture convention:** `djs-create-model` Step 6 creates a `<model_lower>` pytest
...
`<package_name>/<app_name>/tests/recipes.py` (e.g. because `djs-create-model`
...
If `djs-create-model` already created a recipe with field declarations, use that
...
> Run `/djs-doc <app_name>` to add the new views and any permission rules to the app
> README.

Do not run `/djs-doc` automatically.
