#!/usr/bin/env bash
# Fetch kubeconfig from the K3s server and write it to ~/.kube/{{cookiecutter.project_slug}}.yaml
...
KUBECONFIG_PATH="${KUBECONFIG:-$HOME/.kube/{{cookiecutter.project_slug}}.yaml}"
