...
Read `docs/CronJobs.md` before proceeding.
...
> (`/djstudio create-command <app_name> <command_name>`)
...
be created when the project is first deployed via `/djstudio launch`.
...
Checks the command exists (offers `create-command` if not). Reads the command's
...
Example:
  /djstudio create-cron orders process_pending_refunds
