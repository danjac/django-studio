...
> `helm/site/values.secret.yaml` not found. Run `/djs-deploy` first to set
...
```bash
new_admin_url="$(.agents/skills/scripts/random-slug.py)/"
```
...
If **manual**, stop and tell the user to re-run `/djs-rotate-secrets` when ready.
...
NAMESPACE="$namespace" \
  .agents/skills/djs-rotate-secrets/scripts/patch-k8s-secrets.sh
```
...
> `djangoSecretKeyFallbacks` by running `/djs-rotate-secrets` again
