...
| `ForeignKey` / `OneToOneField` | Target model (and app if ambiguous)? `on_delete`? (default `PROTECT` — see `docs/django-models.md#on_delete`; `CASCADE` only for a genuinely owned child, `SET_NULL` only for a nullable informational reference, and either one needs a comment saying why) `related_name`? (suggest `<model_lower>s`) |
